...
world = World()
world.moveplayer(0, 0)
worldviewrect = pygame.Rect(0, 0, WINDOWSIZE[0] - 92, WINDOWSIZE[1] - 20)
worldview = WorldView(world, window)
hudrect = pygame.Rect(WINDOWSIZE[0] - 92, 0, 100, WINDOWSIZE[1])
hud = HUD(world, window)
gameended = False

while not gameended:
    gameended = handleevents()
    worldviewrect.width = window.get_width() - 92
    worldviewrect.height = window.get_height() - 20
    hudrect.left = window.get_width() - 92
