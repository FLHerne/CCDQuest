...
def draw(drawsurface, string, region, size, color, font=defaultfont, xcentered=True, ycentered=True, beveled=False):
    ...
    darkcolor = maincolor.correct_gamma(10)
    lightcolor = BLACK # maincolor.correct_gamma(0.1)
    for (xbeveloffset, ybeveloffset, color) in ((1, 1, darkcolor), (1, -1, darkcolor), (-1, 1, darkcolor), (-1, -1, lightcolor), (0, 0, maincolor)) if beveled else ((0, 0, color),):
